%% Voltage Imaging Mapping Analysis 050926 newDFFcalc and MCfineROI (laser row artifact) — parallel MC pass 2 + parallel dF/F trials
...
% VoltImg_mapping_parallel_dffOneTrial.
%
...
useParallelMcPass2 = true;   % parfor NoRMCorre pass 2 after global template is built
useParallelDffTrials = true; % parfor over trials in dF/F section
...
savePath = '/Volumes/phoenixinthesky/Masato/Voltage Imaging Data_Phoenix/voltMapping/MC Imaging Data Parallel';
...
if useParallelMcPass2
    parfor tt = 1:length(imagesIndex)
...
if useParallelDffTrials
    parfor tt = 1:nTrials
        trialDffPack{tt} = VoltImg_mapping_parallel_dffOneTrial(tt, dffCtx);
    end
else
    for tt = 1:nTrials
...
maxNumFrames = 0;
for tt = 1:nTrials
    maxNumFrames = max(maxNumFrames, trialDffPack{tt}.numFrames);
end
for nn = 1:nCells
