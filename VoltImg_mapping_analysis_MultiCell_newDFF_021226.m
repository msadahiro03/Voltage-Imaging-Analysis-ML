%% Voltage Imaging Mapping Analysis 021226
...
ImgsFilePath = cchar(uigetdir('/Volumes/phoenixinthesky/Masato/Voltage Imaging Data_Phoenix/voltMapping/Raw Imaging Data'));
...
if length(imagesIndex) < 100
    ...
    randTrialsForMask = randperm(length(imagesIndex), ceil(length(imagesIndex)*0.50)); % Select random n trials/sweeps to be used to generate an average z-stacked image;
end
...
for tt = randTrialsForMask
    ...
    Y = single(imageStack);
    [d1,d2,~] = size(Y);
    options_rigid_mask = NoRMCorreSetParms('d1',d1,'d2',d2,'bin_width',20,'max_shift',4,'us_fac',50);
    [M1_mask, ~] = normcorre(Y, options_rigid_mask);
    imageStack_mc = M1_mask;
    %%%%%%%%%%%%%%%%
    toc
    
    imageStackMean = zeros(info(1).Height, info(1).Width);
    % imageStackMean = mean(imageStack(:, :, :), 3);
    imageStackMean = mean(imageStack_mc, 3);
    % for ff = 1:size(imageStack_mc, 3); 
    %     figure(101010+tt); 
    %     imagesc(imageStack_mc(:, :, ff)); 
    %     caxis([min(min(min(imageStack_mc(:,:,:)))), max(max(max(imageStack_mc(:,:,:))))]); 
    %     axis equal; axis image; pause; 
    % end
...
for nn = 1:nCells
    ...
    vals = nonzeros(roiMaxStackRidge);
    thr  = prctile(vals, 50);  % keep only top 20% ridge strengths (raise % to be stricter)
    ...
    % 
    % subplot(6,1,4)
    % imagesc(roiMaxStackRidgeReduced); axis equal; axis image; set(gca, 'fontsize', 12);
    ...
    ringWidth   = 3;   % thickness of neuropil ring
...
for tt = 1:nTrials %size(vsTest_inputs, 2)
    ...
    %%%%%%%%%%%%%%%% NoRMCorre motion correction for this trial
    Y = single(imageStack);                % NoRMCorre expects single
    [d1,d2,T] = size(Y);

    % Basic rigid options; tune max_shift/bin_width for your data
    % d1,d2 are height and width of the image respectively. d3 is the number of frames.
    options_rigid = NoRMCorreSetParms( ...
        'd1', d1, ...
        'd2', d2, ...
        'bin_width', 20, ...      % number of frames for template updates
        'max_shift', 4, ...      % max pixels allowed to shift (x,y)
        'us_fac', 50);            % subpixel upsampling

    % Perform motion correction on this trial
    [M1, shifts1, template1] = normcorre(Y, options_rigid);

    % Use motion-corrected movie from here on
    imageStack_mc = M1;
    clear imageStack; clear Y M1;
    imageStack = imageStack_mc;
    %%%%%%%%%%%%%%%%
    ...
    for nn = 1:nCells
        ...
        roiMeanFCorrected = roiMeanF - alphaScalar*bkgrndMeanF; % Corrected trace this trial for this cell
