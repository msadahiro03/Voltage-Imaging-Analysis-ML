...
for tt = 1:nTrials
    if ismember(tt, excludeTrials)
        ...
        for hh = 1:nHolos(voltMapping.trialCond(tt, 1))
            holoSortedDataAllTrials{voltMapping.trialCond(tt, 1)}{hh, 1}(:, tt) = NaN((ipi*nPulses+(preStimWindow+postStimWindow))/1000*Fs+1, 1);
        end
        continue
    end
...
maxDvStack = [];
for tt = 1:100 %length(imagesIndex)
...
%         currFrame = imageStack(:,:,frameIndex);
%         [cleanFrame] = VoltImg_mapping_removeArtifact(currFrame);
%         imageStack(:,:,frameIndex) = cleanFrame;
    end
...
%    Extract the pixel intensity values (new option 2)    
    ...
    df = (df - nanmean(df(ceil(imagingFreq*f0Start):floor(imagingFreq*f0End))))/nanmean(df(ceil(imagingFreq*f0Start):floor(imagingFreq*f0End)))*100;
