function pack = VoltImg_mapping_parallel_dffOneTrial_commonEarlyF0(tt, ctx)
%VOLTIMG_MAPPING_PARALLEL_DFFONETRIAL_COMMONEARLYF0  Fine ROI + dF/F with shared early-trial F0.
%
% Same pipeline as VoltImg_mapping_parallel_dffOneTrial, except F0 for every
% hologram in a trial uses one baseline for that trial/cell: within the last
% ctx.commonEarlyF0BaselineMs milliseconds before the first pulse of the first
% hologram, a rolling window of ctx.commonEarlyF0RollingWinMs selects the
% sub-interval with minimum variance; F0 is the mean of that sub-interval (raw
% and filtered traces separately).
...
baselineMs = ctx.commonEarlyF0BaselineMs;
rollingMs = ctx.commonEarlyF0RollingWinMs;
...
for nn = 1:ctx.nCells
    ...
    f0ThisTrialRaw = NaN;
    f0ThisTrialFilt = NaN;
    if ~pack.isExcluded && ~isempty(fstVecTrial) && numel(fstVecTrial) >= 1
        iHoloLo1 = floor((fstVecTrial(1) - ctx.preStimWindow / 1000) * ctx.imagingFreq);
        preStimRelLast = (ctx.preStimWindow / 1000 * ctx.imagingFreq) - 1;
        nPreStim = numel(1:preStimRelLast);
        firstPulseIdx = iHoloLo1 + nPreStim;
        firstPulseIdx = max(1, min(firstPulseIdx, numFrames));

        nBaselineFr = max(1, round(baselineMs / 1000 * ctx.imagingFreq));
        iBaselineHi = firstPulseIdx - 1;
        iBaselineLo = max(1, firstPulseIdx - nBaselineFr);
        nRollFr = max(3, round(rollingMs / 1000 * ctx.imagingFreq));

        f0ThisTrialRaw = VoltImg_commonEarlyF0_minVarMean(roiMeanFCorrected, iBaselineLo, iBaselineHi, nRollFr);
        f0ThisTrialFilt = VoltImg_commonEarlyF0_minVarMean(roiMeanFCorrectedFilt, iBaselineLo, iBaselineHi, nRollFr);
    end
    ...
    for hh = 1:nHolosThis
        ...
        if ~pack.isExcluded
            ...
            f0FiltThisHolo = f0ThisTrialFilt;
...
function mu = VoltImg_commonEarlyF0_minVarMean(trace, iLo, iHi, nRollFr)
% Mean over the lowest-variance rolling sub-window within [iLo,iHi].
trace = trace(:);
nMax = numel(trace);
iLo = max(1, min(round(iLo), nMax));
iHi = max(1, min(round(iHi), nMax));
if iHi < iLo
    mu = NaN;
    return
end
