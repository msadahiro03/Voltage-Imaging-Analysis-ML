%% Voltage Imaging Mapping Analysis 021226 newDFFcalc and MCfineROI (laser row artifact)
...
%    separate folder.
% 2) maxDvStack built from motion-corrected trial means (non-excluded trials only; third dim is not trial index).
...
% 5) F, F0, dF, dF/F0 calculated from motion-corrected stacks using the
%    per-trial fine ROIs.
...
useLaserRowArtifactFilter = true;
laserArtifactGateColFirst = 175;
laserArtifactGateColLast  = 325;
...
laserArtifactMcSecondSweepForDff = true; % If true: after MC, save HxT bad-row mask; dF/F omits those ROI/neuropil pixels per frame.

% --- MC perf knobs (CLAUDE) ---
mcTiffCompression = Tiff.Compression.None;   % None for fast scratch writes; LZW to shrink files (~2-4x slower writes).
mcScratchDir      = '';                      % If '', auto-set to a *local* tempdir below (NOT the network mount; SMB causes HDF5 v7.3 read/write races).
mcKeepScratch     = false;                   % Delete scratch cache after MC stage finishes.
mcUseParfor       = true;                    % Parallelize the second MC sweep (NoRMCorre + write _mc.tif). Requires Parallel Computing Toolbox.
...
% Read first TIFF metadata for image size; maxDvStack is now built after sweep 2 below.
...
% =====================================================================
% CLAUDE: merged MC pipeline.
% Sweep 1 — read each raw TIFF once, apply laser-row artifact filter,
%   write the cleaned single-precision stack to a scratch directory, and
%   accumulate the global-template mean (skipping excluded trials only
%   for the template, not for the scratch save).
% Sweep 2 — load the cleaned stack from scratch (no raw re-read, no
%   second laser-clean), run NoRMCorre against the shared global
%   template, compute the bad-row mask, and write _mc.tif.
%   Optionally parfor-parallelized.
% =====================================================================

if isempty(mcScratchDir)
    % CLAUDE: keep scratch on a LOCAL filesystem to avoid SMB/HDF5 write races
    % observed on /Volumes/phoenixinthesky/... (parfor worker hit "Cannot read file" on _clean.mat).
    mcScratchDir = fullfile(tempdir, ['VoltImg_mcScratch_', expID]);
end
if ~exist(mcScratchDir, 'dir')
    mkdir(mcScratchDir);
end
% Sanity: warn loudly if scratch is on a network volume.
if startsWith(mcScratchDir, '/Volumes/') || startsWith(mcScratchDir, '\\')
    warning('VoltImg:mcScratchOnNetwork', ...
        ['mcScratchDir is on a network volume (%s). HDF5 v7.3 .mat reads/writes ', ...
         'race over SMB and have already caused parfor failures. Strongly recommend a local path.'], mcScratchDir);
end
disp(['MC scratch dir: ', mcScratchDir]);
...
% Per-trial scratch metadata so Sweep 2 doesn't have to reprobe TIFFs.
nImg = length(imagesIndex);
scratchPaths      = cell(nImg, 1);
trialDims         = nan(nImg, 3);   % [H W nKeep] per trial
nTemplateTrials   = 0;

% ---- Sweep 1: read raw, clean, save cleaned single stack to scratch, accumulate template ----
for tt = 1:nImg
    disp(['MC sweep 1 (clean+template) trial ', num2str(tt), ' of ', num2str(nImg)]);
    ...
    cleanedStack = single(imageStack);

    % Save cleaned stack to scratch as a v7.3 .mat (random-access, no per-frame TIFF setTag overhead).
    [~, rawNameSweep1, ~] = fileparts(ImgfolderContents(imagesIndex(tt)).name);
    scratchFile = fullfile(mcScratchDir, [rawNameSweep1, '_clean.mat']);
    scratchPaths{tt} = scratchFile;
    trialDims(tt, :) = [H, W, nKeep];
    save(scratchFile, 'cleanedStack', '-v7.3');
    % CLAUDE: verify the file is readable before moving on. Network volumes (SMB)
    % occasionally claim a write succeeded but the file is not yet visible/sane.
    okScratch = false;
    for sanityTry = 1:5
        if exist(scratchFile, 'file') == 2
            try
                whos('-file', scratchFile, 'cleanedStack'); %#ok<NOPRT>
                okScratch = true;
                break
            catch
                pause(0.5);
    ...
    if ~okScratch
        error('VoltImg:scratchWriteFailed', ...
            'Could not verify scratch file %s after save. Check disk space and that mcScratchDir is local.', scratchFile);
    end

    if ~ismember(tt, excludeTrials)
        if mcUseGateColumnsOnly
            globalTemplateAccum = globalTemplateAccum + mean(cleanedStack(:, gateColFirstMc:gateColLastMc, :), 3);
        else
            globalTemplateAccum = globalTemplateAccum + mean(cleanedStack, 3);
        end
...
% ---- Sweep 2: NoRMCorre + write _mc.tif, optionally parfor ----
imgNames                 = {ImgfolderContents(imagesIndex).name};
trialMeans_mc            = cell(nImg, 1);   % per-trial mean image of motion-corrected stack
gateColFirstSweep2       = max(1, min(laserArtifactGateColFirst, laserArtifactGateColLast));

% Decide parfor on/off
useParforHere = mcUseParfor;
if useParforHere
    try
        useParforHere = useParforHere && license('test', 'Distrib_Computing_Toolbox');
    catch
        useParforHere = false;
    end
end

% Snapshot plain locals so parfor body is transparently sliced.
locScratchPaths       = scratchPaths;
locImgNames           = imgNames;
locMcTiffFolder       = mcTiffFolder;
locGlobalTemplate     = globalTemplate;
locUseGateColsOnly    = mcUseGateColumnsOnly;
locGateColFirst       = laserArtifactGateColFirst;
locGateColLast        = laserArtifactGateColLast;
locThreshMode         = laserArtifactThreshMode;
locThreshParam        = laserArtifactThreshParam;
locWriteBadRows       = laserArtifactMcSecondSweepForDff;
locTiffCompression    = mcTiffCompression;
locNormcorrePath      = normcorrePath;

if useParforHere
    parfor tt = 1:nImg
        trialMeans_mc{tt} = sweepTwoOneTrial( ...
            tt, locScratchPaths{tt}, locImgNames{tt}, locMcTiffFolder, locGlobalTemplate, ...
            locUseGateColsOnly, locGateColFirst, locGateColLast, ...
            locThreshMode, locThreshParam, locWriteBadRows, locTiffCompression, ...
            locNormcorrePath);
    end
else
    for tt = 1:nImg
        disp(['MC sweep 2 (normcorre+save) trial ', num2str(tt), ' of ', num2str(nImg)]);
        trialMeans_mc{tt} = sweepTwoOneTrial( ...
            tt, locScratchPaths{tt}, locImgNames{tt}, locMcTiffFolder, locGlobalTemplate, ...
            locUseGateColsOnly, locGateColFirst, locGateColLast, ...
            locThreshMode, locThreshParam, locWriteBadRows, locTiffCompression, ...
...
% Assemble maxDvStack from non-excluded trials only (matches original semantics).
maxDvStack = zeros(infoFirst(1).Height, infoFirst(1).Width, nImg);
maskCounter = 0;
for tt = 1:nImg
    if ismember(tt, excludeTrials)
        continue
    ...
    maxDvStack(:, :, maskCounter) = trialMeans_mc{tt};
end
if maskCounter < size(maxDvStack, 3)
    maxDvStack(:, :, maskCounter+1:end) = [];
end

% Cleanup scratch unless caller asked to keep it.
if ~mcKeepScratch
    for tt = 1:nImg
        if exist(scratchPaths{tt}, 'file') == 2
            delete(scratchPaths{tt});
        end
    end
    try
        rmdir(mcScratchDir);
    catch
        % directory may not be empty if other artefacts were written; ignore.
    end
end
...
voltMapping.mcPerf.tiffCompression = mcTiffCompression;
voltMapping.mcPerf.scratchDir      = mcScratchDir;
voltMapping.mcPerf.usedParfor      = useParforHere;
...
% --- CLAUDE preallocation: per (cond, holo) sizes + a column write counter. ---
% Replaces O(N^2) concatenation of `[old, vec]` per trial with indexed assignment.
nTrialsPerCond = zeros(nConds, 1);
for ttPre = 1:nTrials
    nTrialsPerCond(voltMapping.trialCond(ttPre, 1)) = nTrialsPerCond(voltMapping.trialCond(ttPre, 1)) + 1;
end
holoWindowLen = ceil((ipi*nPulses + (preStimWindow + postStimWindow))/1000*imagingFreq) + 2;
holoWriteIdx = cell(nCells, 1);
for nn = 1:nCells
    holoWriteIdx{nn} = cell(nConds, 1);
    for cc = 1:nConds
        holoWriteIdx{nn}{cc} = zeros(nHolos(cc), 1);
        for hh = 1:nHolos(cc)
            analysisStruct.(F0CellNames{nn}){cc}{hh, 1}                    = nan(1, nTrialsPerCond(cc));
            analysisStruct.(holoSortedImagingCellNames{nn}){cc}{hh, 1}     = nan(holoWindowLen, nTrialsPerCond(cc));
            analysisStruct.(filtHoloSortedImagingCellNames{nn}){cc}{hh, 1} = nan(holoWindowLen, nTrialsPerCond(cc));
        end
    end
end

% --- CLAUDE: hoisted Butterworth coefficients (identical across all trials/cells). ---
cutOffFreqIm = 40;
...
for tt = 1:nTrials
    ...
    %--------------------------------------------------------------
    % Pass 1: compute per-trial fine ROI for all cells
    %--------------------------------------------------------------
    % CLAUDE: hoist trial-level (cell-independent) Gaussian + percentile prep
    % out of the cell loop. These quantities depend only on meanImgThisTrial.
    roiMaxStackDouble_trial = im2double(meanImgThisTrial);
    roiPixels_trial = roiMaxStackDouble_trial > 0;
    roiMaxStackFilt_trial = imgaussfilt(roiMaxStackDouble_trial, 0.7);
    vals_trial = roiMaxStackFilt_trial(roiPixels_trial);
    if ~isempty(vals_trial)
        lo_trial = prctile(vals_trial, 10);
        hi_trial = prctile(vals_trial, 99);
        roiMaxStackNorm_trial = (roiMaxStackFilt_trial - lo_trial) / max(hi_trial - lo_trial, eps);
        roiMaxStackNorm_trial = min(max(roiMaxStackNorm_trial, 0), 1);
    else
        roiMaxStackNorm_trial = roiMaxStackFilt_trial;
    end
    ...
    for nn = 1:nCells
        ...
        if isempty(fineRoiXAllCells{nn}{tt})
            % CLAUDE: cell-specific bindings reuse the hoisted trial-level prep.
            roiMaxStackDouble = roiMaxStackDouble_trial;
            roiMaxStackFilt   = roiMaxStackFilt_trial;
            roiMaxStackNorm   = roiMaxStackNorm_trial;
    ...
    % Build a trial-wide ROI mask so neuropil excludes ROI pixels of any cell.
    ...
    %--------------------------------------------------------------
    % Pass 2: extract F traces and trial-specific neuropil traces
    %--------------------------------------------------------------
    counter2 = 0;
    for nn = 1:nCells
        counter2 = counter2+1;
        ...
        % CLAUDE: always route through the vectorized helper. Pass an empty
        % bad-row mask when the second-sweep mask is disabled, instead of using
        % the buggy Cartesian-indexing fallback that previously returned a
        % rectangular sub-block rather than the list of ROI pixels.
        if laserArtifactMcSecondSweepForDff
            roiMeanF = VoltImg_roiMeanPerFrameExcludeBadRows_claude(imageStack, ...
                fineRoiXAllCells{nn}{tt}, fineRoiYAllCells{nn}{tt}, badRowMaskMc);
        else
            roiMeanF = VoltImg_roiMeanPerFrameExcludeBadRows_claude(imageStack, ...
                fineRoiXAllCells{nn}{tt}, fineRoiYAllCells{nn}{tt}, []);
        end

        % Trial-specific neuropil ring for this cell, excluding any ROI pixel.
        innerBuffer = 2;
        ringWidth   = 3;
        minArea     = 50;
        ...
        % CLAUDE: same vectorized-helper routing as the ROI path above.
        if laserArtifactMcSecondSweepForDff
            bkgrndMeanF = VoltImg_roiMeanPerFrameExcludeBadRows_claude(imageStack, ...
                bkgrndRoiXTrial, bkgrndRoiYTrial, badRowMaskMc);
        else
            bkgrndMeanF = VoltImg_roiMeanPerFrameExcludeBadRows_claude(imageStack, ...
                bkgrndRoiXTrial, bkgrndRoiYTrial, []);
        end
        ...
        % CLAUDE: bIm/aIm hoisted to before the trial loop; nothing to recompute here.
        % Filter full-trial corrected F once; holo-window filt dF/F uses same windows/indices as unfiltered.
        if ~ismember(tt, excludeTrials)
        ...
        for hh = 1:nHolos(voltMapping.trialCond(tt, 1))
            ...
            if ~ismember(tt, excludeTrials)            
                ...
                roiFCorrectedThisHoloPreStimFilt = roiFCorrectedThisHoloFilt(1:(preStimWindow/1000*imagingFreq)-1);
            ...
            % CLAUDE: indexed write into preallocated NaN matrices instead of [old, vec] concat.
            ccTtl = voltMapping.trialCond(tt, 1);
            hhSlot = holoSeqThisTrial(hh);
            holoWriteIdx{nn}{ccTtl}(hhSlot) = holoWriteIdx{nn}{ccTtl}(hhSlot) + 1;
            colIdx = holoWriteIdx{nn}{ccTtl}(hhSlot);
            if ismember(tt, excludeTrials)
                analysisStruct.(F0CellNames{nn}){ccTtl}{hhSlot, 1}(1, colIdx) = NaN;
                % holoSortedImagingCellNames / filt: columns already NaN-initialized — leave alone.
            else
                analysisStruct.(F0CellNames{nn}){ccTtl}{hhSlot, 1}(1, colIdx) = f0ThisHolo;
                vecLen = numel(dFFThisHolo);
                if vecLen > holoWindowLen
                    vecLen = holoWindowLen;
                end
                analysisStruct.(holoSortedImagingCellNames{nn}){ccTtl}{hhSlot, 1}(1:vecLen, colIdx)     = dFFThisHolo(1:vecLen);
                analysisStruct.(filtHoloSortedImagingCellNames{nn}){ccTtl}{hhSlot, 1}(1:vecLen, colIdx) = filtdffThisHolo(1:vecLen);
            end
        end
...
% =====================================================================
% CLAUDE local helper: per-trial body of MC sweep 2.
% Loads cleaned single-precision stack from scratch, runs NoRMCorre,
% computes the bad-row mask, and writes the _mc.tif (and _mc_badRows.mat).
% Returns the per-trial mean of the motion-corrected stack.
% =====================================================================
function imgMean = sweepTwoOneTrial(tt, scratchFile, rawTiffName, mcTiffFolder, ...
    globalTemplate, useGateColsOnly, gateColFirst, gateColLast, ...
    threshMode, threshParam, writeBadRows, tiffCompression, normcorrePath)

    if nargin >= 13 && ~isempty(normcorrePath) && exist(normcorrePath, 'dir') == 7
        addpath(normcorrePath);
    end
    addpath(fileparts(mfilename('fullpath')));   % ensure VoltImg_* helpers are on worker path

    % CLAUDE: retry the scratch load — SMB-mounted scratch dirs can transiently
    % return "Cannot read file" right after a remote save flush.
    S = [];
    lastErr = [];
    for tryIdx = 1:5
        if exist(scratchFile, 'file') == 2
            try
                S = load(scratchFile, 'cleanedStack');
                break
            catch err
                lastErr = err;
                pause(1);
    ...
    if isempty(S)
        if ~isempty(lastErr)
            error('VoltImg:scratchReadFailed', ...
                'Trial %d: scratch load failed after retries (%s): %s', tt, scratchFile, lastErr.message);
        else
            error('VoltImg:scratchReadFailed', ...
                'Trial %d: scratch file not found after retries (%s).', tt, scratchFile);
        end
    end
    cleanedStack = S.cleanedStack;
    [Hloc, Wloc, ~] = size(cleanedStack);

    if useGateColsOnly
        gcF = max(1, min(gateColFirst, gateColLast));
        gcL = min(Wloc, max(gateColFirst, gateColLast));
        Ygate = cleanedStack(:, gcF:gcL, :);
        [d1Gate, d2Gate, ~] = size(Ygate);
        options_gate = NoRMCorreSetParms('d1', d1Gate, 'd2', d2Gate, ...
            'bin_width', 15, 'max_shift', 4, 'us_fac', 50, 'init_batch', 1);
        [~, shifts] = normcorre(Ygate, options_gate, globalTemplate);

        [d1, d2, ~] = size(cleanedStack);
        options_full = NoRMCorreSetParms('d1', d1, 'd2', d2, ...
            'bin_width', 15, 'max_shift', 4, 'us_fac', 50, 'init_batch', 1);
        imageStack_mc = apply_shifts(cleanedStack, shifts, options_full);
    else
        [d1, d2, ~] = size(cleanedStack);
        options_rigid = NoRMCorreSetParms('d1', d1, 'd2', d2, ...
            'bin_width', 15, 'max_shift', 4, 'us_fac', 50, 'init_batch', 1);
        [imageStack_mc, ~] = normcorre(cleanedStack, options_rigid, globalTemplate);
    end

    [~, rawName, ~] = fileparts(rawTiffName);
    mcPath = fullfile(mcTiffFolder, [rawName, '_mc.tif']);

    if writeBadRows
        badRowMask = VoltImg_laserRowArtifact_badRowMaskStack(single(imageStack_mc), ...
            gateColFirst, gateColLast, threshMode, threshParam); %#ok<NASGU>
        badRowsMatPath = fullfile(mcTiffFolder, [rawName, '_mc_badRows.mat']);
        try
            save(badRowsMatPath, 'badRowMask', 'gateColFirst', 'gateColLast', ...
                'threshMode', 'threshParam', '-v7.3');
        catch
            save(badRowsMatPath, 'badRowMask', 'gateColFirst', 'gateColLast', ...
                'threshMode', 'threshParam');
        end
    end
    ...
    tagstruct.Compression = tiffCompression;

    nFr = size(imageStack_mc_uint16, 3);
    for k = 1:nFr
