%% Voltage Imaging Mapping Analysis 021226 newDFFcalc and MCfineROI (ephysAligned fork)
% ephysAligned: dual sorted-ephys (all trials vs excludeTrials NaNs), robust
% zeroDummySequence, ephys mirrors trialExcluder masks from paired ROI, pairedRoiIndex.
% New version with:
% 1) Per-trial NoRMCorre motion correction of raw image stacks, saved to a
%    separate folder.
% 2) maxDvStack: per-trial mean planes (NaN for ephys-excluded and trials not in the random 50%% subsample);
%    each plane uses first max(1,floor(imagingFreq*4)) MC frames; third dim is trial index; grand mean omitnan.
...
ephysFilePath = char(uigetdir('/Volumes/phoenixinthesky/Masato/Voltage Imaging Data_Phoenix/voltMapping/DAQ_Ephys Data'));
...
ImgsFilePath = char(uigetdir('/Volumes/phoenixinthesky/Masato/Voltage Imaging Data_Phoenix/voltMapping/Raw Imaging Data'));
...
if ~exist('voltMapping.sampleFreq', 'var')
    imagingFreq = 330.28;
else    
    imagingFreq = voltMapping.sampleFreq;
end
Fs = voltMapping.daqParams.Fs;
if ~exist('voltMapping.daqParams.maxSweepLengthSec', 'var')
    trialTime = voltMapping.daqParams.sweepLengthSec;
else
...
holoSortedDataAllTrials = cell(nConds, 1);              % NaN columns for excludeTrials (matches imaging)
holoSortedDataAllTrials_incAllTrials = cell(nConds, 1); % all trials, no excludeTrials NaNing
for cc = 1:nConds
    holoSortedDataAllTrials{cc} = cell(nHolos(cc), 1);
    holoSortedDataAllTrials_incAllTrials{cc} = cell(nHolos(cc), 1);
end
...
% Dummy hologram sequence for trials with no sequence (e.g. 0 mW / no stim).
zeroDummySequence = [];
if any(voltMapping.outParams.power == 0)
    zeroDummySequence = voltMapping.outParams.sequence{1, 2};
end
if isempty(zeroDummySequence)
    seqCell = voltMapping.outParams.sequence;
    for zix = 1:numel(seqCell)
        if ~isempty(seqCell{zix})
            zeroDummySequence = seqCell{zix};
            break;
        end
    end
end
if isempty(zeroDummySequence)
    error('ephysAligned: could not define zeroDummySequence; check voltMapping.outParams.sequence.');
end
...
for tt = 1:nTrials
    ...
    for hh = 1:nHolos(voltMapping.trialCond(tt, 1))
        ...
        ccT = voltMapping.trialCond(tt, 1);
        holoSlot = holoSeqIndex{ccT}(hh, end);
        holoSortedDataAllTrials_incAllTrials{ccT}{holoSlot, 1} = [holoSortedDataAllTrials_incAllTrials{ccT}{holoSlot, 1}, thisHoloSweep];
        if ismember(tt, excludeTrials)
            holoSortedDataAllTrials{ccT}{holoSlot, 1} = [holoSortedDataAllTrials{ccT}{holoSlot, 1}, nan(size(thisHoloSweep))];
        else
            holoSortedDataAllTrials{ccT}{holoSlot, 1} = [holoSortedDataAllTrials{ccT}{holoSlot, 1}, thisHoloSweep];
        end
    end
end

% Trial-averaged ephys per hologram: (1) all trials, (2) excludeTrials NaNed (matches imaging)
holoSortedDataMean_incAllTrials = cell(nConds, 1);
holoSortedDataMean = cell(nConds, 1);
for cc = 1:nConds
    for hh = 1:nHolos(cc)
        holoSortedDataMean_incAllTrials{cc}(:, hh) = nanmean(holoSortedDataAllTrials_incAllTrials{cc}{hh}, 2);
        holoSortedDataMean{cc}(:, hh) = nanmean(holoSortedDataAllTrials{cc}{hh}, 2);
    end
end

CIephysAllConds_incAllTrials = cell(nConds, 1);
for cc = 1:nConds
    for hh = 1:nHolos(cc)
        confidence_level = 0.95;
        trialsMat = holoSortedDataAllTrials_incAllTrials{cc}{hh, 1};
        means = nanmean(trialsMat, 2);
        std_errors = std(trialsMat, 0, 2, "omitnan") / sqrt(size(trialsMat, 2));
        nCol = size(trialsMat, 2);
        if nCol < 2
            margin_of_error = nan(size(means));
        else
            t_score = tinv((1 + confidence_level) / 2, nCol - 1);
            margin_of_error = t_score * std_errors;
        end
        lower_bounds = means - margin_of_error;
        upper_bounds = means + margin_of_error;
        if UpOrDown == '2'
            CIephysAllConds_incAllTrials{cc}{hh, 1} = [lower_bounds, upper_bounds];
        elseif UpOrDown == '1'
            CIephysAllConds_incAllTrials{cc}{hh, 1} = [-lower_bounds, -upper_bounds];
        end
    end
...
for cc = 1:nConds
    for hh = 1:nHolos(cc)
        confidence_level = 0.95;
        trialsMat = holoSortedDataAllTrials{cc}{hh, 1};
        means = nanmean(trialsMat, 2);
        std_errors = std(trialsMat, 0, 2, "omitnan") / sqrt(size(trialsMat, 2));
        nCol = size(trialsMat, 2);
        if nCol < 2
...
voltMapping.ephys.holoSortedDataAllTrials_incAllTrials = holoSortedDataAllTrials_incAllTrials;
voltMapping.ephys.holoSortedDataAllTrials             = holoSortedDataAllTrials;
voltMapping.ephys.nPulseCoords                        = nPulseCoords;
voltMapping.ephys.CIephysAllConds_incAllTrials        = CIephysAllConds_incAllTrials;
voltMapping.ephys.CIephysAllConds                     = CIephysAllConds;
voltMapping.ephys.holoSortedDataMean_incAllTrials     = holoSortedDataMean_incAllTrials;
...
voltMapping.mouseID = ['voltMapping_Analysis_', expID, '_MultiCellAnalysis_MCfineROI_ephysAligned'];
...
if ePhysAvail == 1
    if nCells == 1
        voltMapping.ephys.pairedRoiIndex = 1;
    else
        voltMapping.ephys.pairedRoiIndex = input('ROI index paired with whole-cell ephys (1..nCells): ');
        assert(voltMapping.ephys.pairedRoiIndex == fix(voltMapping.ephys.pairedRoiIndex) && ...
            voltMapping.ephys.pairedRoiIndex >= 1 && voltMapping.ephys.pairedRoiIndex <= nCells, ...
            'ephysAligned: pairedRoiIndex must be an integer from 1 to nCells.');
    end
else
    voltMapping.ephys.pairedRoiIndex = NaN;
end
...
for tt = 1:nTrials
    ...
    for nn = 1:nCells
        ...
        for hh = 1:nHolos(voltMapping.trialCond(tt, 1))
            ...
            % Pad with constant dF/F at trace onset so filter() starts in steady state.
            % Default zero initial conditions otherwise pin filtered values (and SEM/CI) near 0 at t=1.
            padLenFilt = max(60, 6 * max(numel(bIm), numel(aIm)));
            dFFcol = dFFThisHolo(:);
            xPad = [repmat(dFFcol(1), padLenFilt, 1); dFFcol];
            yPad = filter(bIm, aIm, xPad);
            filtdffThisHolo = yPad(padLenFilt+1:end);
...
%% Mirror trialExcluder column exclusions onto sorted ephys (paired ROI cell)
exclHoloSortedDataAllTrials = [];
exclFiltHoloSortedDataAllTrials = [];
exclHoloSortedDataMean = [];
exclFiltHoloSortedDataMean = [];
exclCIephysAllConds = [];
exclFiltCIephysAllConds = [];

if ePhysAvail == 1 && isscalar(voltMapping.ephys.pairedRoiIndex) && ...
        ~isnan(voltMapping.ephys.pairedRoiIndex) && voltMapping.ephys.pairedRoiIndex >= 1 && ...
        voltMapping.ephys.pairedRoiIndex <= nCells
    paired = round(voltMapping.ephys.pairedRoiIndex);
    exclHoloSortedDataAllTrials = cell(nConds, 1);
    exclFiltHoloSortedDataAllTrials = cell(nConds, 1);
    for cc = 1:nConds
        exclHoloSortedDataAllTrials{cc} = cell(nHolos(cc), 1);
        exclFiltHoloSortedDataAllTrials{cc} = cell(nHolos(cc), 1);
        for hh = 1:nHolos(cc)
            M = holoSortedDataAllTrials{cc}{hh};
            exclHoloSortedDataAllTrials{cc}{hh} = M;
            exclFiltHoloSortedDataAllTrials{cc}{hh} = M;
            imExH = analysisStruct.(exclHoloSortedImagingAllTrialsCellNames{paired}){cc}{hh};
            imExF = analysisStruct.(exclFiltHoloSortedImagingAllTrialsCellNames{paired}){cc}{hh};
            nCol = size(M, 2);
            for tt = 1:nCol
                if tt <= size(imExH, 2) && all(isnan(imExH(:, tt)))
                    exclHoloSortedDataAllTrials{cc}{hh}(:, tt) = nan(size(M, 1), 1);
                end
                if tt <= size(imExF, 2) && all(isnan(imExF(:, tt)))
                    exclFiltHoloSortedDataAllTrials{cc}{hh}(:, tt) = nan(size(M, 1), 1);
                end
            end
        end
    end

    exclHoloSortedDataMean = cell(nConds, 1);
    exclFiltHoloSortedDataMean = cell(nConds, 1);
    for cc = 1:nConds
        for hh = 1:nHolos(cc)
            exclHoloSortedDataMean{cc}(:, hh) = nanmean(exclHoloSortedDataAllTrials{cc}{hh}, 2);
            exclFiltHoloSortedDataMean{cc}(:, hh) = nanmean(exclFiltHoloSortedDataAllTrials{cc}{hh}, 2);
        end
    end

    exclCIephysAllConds = cell(nConds, 1);
    for cc = 1:nConds
        for hh = 1:nHolos(cc)
            confidence_level = 0.95;
            trialsMat = exclHoloSortedDataAllTrials{cc}{hh, 1};
            means = nanmean(trialsMat, 2);
            std_errors = std(trialsMat, 0, 2, "omitnan") / sqrt(size(trialsMat, 2));
            nCol = size(trialsMat, 2);
            if nCol < 2
            ...
            upper_bounds = means + margin_of_error;
            if UpOrDown == '2'
                exclCIephysAllConds{cc}{hh, 1} = [lower_bounds, upper_bounds];
            elseif UpOrDown == '1'
                exclCIephysAllConds{cc}{hh, 1} = [-lower_bounds, -upper_bounds];
            end
        end
    ...
    exclFiltCIephysAllConds = cell(nConds, 1);
    for cc = 1:nConds
        for hh = 1:nHolos(cc)
            confidence_level = 0.95;
            trialsMat = exclFiltHoloSortedDataAllTrials{cc}{hh, 1};
            means = nanmean(trialsMat, 2);
            std_errors = std(trialsMat, 0, 2, "omitnan") / sqrt(size(trialsMat, 2));
            nCol = size(trialsMat, 2);
            if nCol < 2
            ...
            upper_bounds = means + margin_of_error;
            if UpOrDown == '2'
                exclFiltCIephysAllConds{cc}{hh, 1} = [lower_bounds, upper_bounds];
            elseif UpOrDown == '1'
                exclFiltCIephysAllConds{cc}{hh, 1} = [-lower_bounds, -upper_bounds];
            end
        end
    ...
    voltMapping.ephys.exclHoloSortedDataAllTrials     = exclHoloSortedDataAllTrials;
    voltMapping.ephys.exclFiltHoloSortedDataAllTrials  = exclFiltHoloSortedDataAllTrials;
    voltMapping.ephys.exclHoloSortedDataMean           = exclHoloSortedDataMean;
    voltMapping.ephys.exclFiltHoloSortedDataMean       = exclFiltHoloSortedDataMean;
    voltMapping.ephys.exclCIephysAllConds              = exclCIephysAllConds;
    voltMapping.ephys.exclFiltCIephysAllConds        = exclFiltCIephysAllConds;
end
...
if ePhysAvail == 1 && isfield(voltMapping.ephys, 'pairedRoiIndex') && ...
        ~isnan(voltMapping.ephys.pairedRoiIndex)
    nn = round(voltMapping.ephys.pairedRoiIndex);
    disp(['Overlay imaging cell (ephys paired ROI): ', num2str(nn)]);
else
...
if ePhysAvail == 1 && isfield(voltMapping.ephys, 'pairedRoiIndex') && ...
        ~isnan(voltMapping.ephys.pairedRoiIndex)
    cellIdx = round(voltMapping.ephys.pairedRoiIndex);
    disp(['Four-panel imaging cell (ephys paired ROI): ', num2str(cellIdx)]);
else
