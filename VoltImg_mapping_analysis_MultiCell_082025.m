...
postStimWindow = 40; % time(ms) to add to stim window after final pulse + ipi
preStimWindow = voltMapping.holoStimParams.nextHoloDelay(1)- postStimWindow; % time(ms) window before first pulse 
...
figure(9); set(gcf, 'Position',  [100, 100, 1800, 400]); clf; 
colormap('winter'); imagesc(maxDvStackBrighter); axis equal; axis image; colorbar; set(gca, 'fontsize', 12); % caxis([-7 -4]);
...
for nn = 1:nCells
    figure(10);
    set(gcf, 'Position',  [100, 100, 1800, 150]);
    clf
...
for nn = 1:nCells
    ...
    clf
    subplot(3,1,1)
    colormap('winter'); imagesc(maxDvStackBrighter); axis equal; axis image; set(gca, 'fontsize', 12);
    hold on;
    ...
    for rr = 1:length(roiXAllCells{nn})
        roiMeanMaxDvStack(roiXAllCells{nn}(rr), roiYAllCells{nn}(rr)) = mean(maxDvStack(roiXAllCells{nn}(rr), roiYAllCells{nn}(rr),:), 3);
    end
...
allRois = zeros(size(maxDvStackBrighter, 1), size(maxDvStackBrighter, 2));
for nn = 1:nCells
    thisRoi = zeros(size(maxDvStackBrighter, 1), size(maxDvStackBrighter, 2));
    for rr = 1:length(roiXAllCells{nn})
        thisRoi(roiXAllCells{nn}(rr), roiYAllCells{nn}(rr)) = mean(maxDvStackBrighter(roiXAllCells{nn}(rr), roiYAllCells{nn}(rr),:), 3);
    end
    allRois = allRois + thisRoi;
end
figure(20); set(gcf, 'Position',  [100, 100, 1800, 100]); clf; 
...
% figure(30); set(gcf, 'Position',  [100, 100, 1800, 400]); clf; 
% colormap('winter'); imagesc(maxDvStackBrighter); axis equal; axis image; colorbar; set(gca, 'fontsize', 12); % caxis([-7 -4]);
% % Hand select cell or area of interest, by freehand drawing
% BkgrndX = []; BkgrndY = [];
% roiHandSelect = drawfreehand;
...
voltMapping.mouseID = ['voltMapping_Analysis_', expID, '_MultiCell'];
...
for nn = 1:nCells
    expID = num2str(mouseID);
    voltMapping.mouseID = ['voltMapping_Analysis_', expID, '_MultiCell', '_Cell', num2str(nn)];
    directory = '/Volumes/ExData2/Voltage Imaging/VoltMapping/Analysis Results';
    fileName = [num2str(voltMapping.mouseID), '.mat'];
