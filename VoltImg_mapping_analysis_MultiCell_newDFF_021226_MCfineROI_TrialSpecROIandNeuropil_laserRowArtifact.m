...
%    separate folder.
% 2) maxDvStack: per-trial mean planes — NaN for ephys-excluded and trials outside random 50%% subsample;
%    third dim is trial index; each plane uses first max(1,floor(imagingFreq*4)) MC frames only. Grand mean omitsnan.
...
mcUseGateColumnsOnly      = false;       % If true, estimate shifts from gate columns only, then apply shifts to full frames.
...
% Preallocate maxDvStack using first TIFF dimensions (third dim = trial index tt)
...
for tt = 1:length(imagesIndex)
    ...
    % Always MC and save _mc.tif so trial tt matches the dF/F loader; excluded trials are
    % omitted only from maxDvStack (rough-ROI reference) below, not from disk output.
    ...
    if ismember(tt, excludeTrials) || ~maxDvTrialMask(tt)
...
%% Crash-safe checkpoint: motion correction complete
% Written under saveDirectory (same folder as Motion_Corrected_Tiffs). Reload
% that .mat (variables include voltMapping, ImgfolderContents, paths) if MATLAB
% exits before rough ROI / dF/F (MC stacks on disk are unchanged).
checkpointAfterMcFile = fullfile(saveDirectory, 'checkpoint_after_motion_correction.mat');
analysisCheckpointTimestamp_MC = datetime('now');
varsCheckpointAfterMC = { ...
    'voltMapping', 'ImgfolderContents', 'normcorrePath', 'mouseID', 'savePath', 'saveDirectory', 'expID', ...
    'nTrials', 'nConds', 'nHolos', 'imagingFreq', 'Fs', 'ipi', 'nPulses', ...
    'preStimWindow', 'postStimWindow', 'startTime', 'UpOrDown', 'excludeTrials', ...
    'powers', 'nextHoloDelay', 'pulseDurs', 'trialTime', ...
    'imagesIndex', 'cutOffFreq', 'blp', 'alp', ...
    'vThreshold', 'mappingInputs', 'mappingInputsBaselined', ...
    ...
    'rawImgNChannels', 'ePhysAvail', 'holoSeqIndex', ...
    'nPulseCoords', 'nPulseCoordsImaging', ...
    'ephysHoloSweepLen', 'ephysHoloSweepLenNominal', 'minAvailAcrossTrials', ...
    'preStimBaselineIdx', 'nFirstPulseSample', 'iLastPreStim', 'preStimBaselineTrimMs', ...
    'analysisCheckpointTimestamp_MC'};
if exist('zeroDummySequence', 'var')
    varsCheckpointAfterMC{end+1} = 'zeroDummySequence';
end
varsCheckpointAfterMC = varsCheckpointAfterMC(cellfun(@(n) exist(n, 'var') == 1, varsCheckpointAfterMC));
try
    save(checkpointAfterMcFile, varsCheckpointAfterMC{:}, '-v7.3');
catch
    try
        save(checkpointAfterMcFile, varsCheckpointAfterMC{:});
    catch ME
        warning('VoltImg:checkpointAfterMC', 'Could not write MC checkpoint: %s', ME.message);
    end
end
...
%% Crash-safe checkpoint: dF/F, means, CIs, and trial-excluder outputs complete
% Same variable set as "Save Analysis Results" (filtered by exist). Use if the
% script stops during later plotting / alignment sections.
...
checkpointAfterDffFile = fullfile(saveDirectory, 'checkpoint_after_dff_calculation.mat');
analysisCheckpointTimestamp_dff = datetime('now');
varsCheckpointAfterDff = { ...
    ...
    'laserArtifactThreshMode', 'laserArtifactThreshParam', 'laserArtifactMcMode', ...
    'mcUseGateColumnsOnly', 'laserArtifactMcSecondSweepForDff', ...
    'cellID', 'analysisCheckpointTimestamp_dff'};
if exist('zeroDummySequence', 'var')
    varsCheckpointAfterDff{end+1} = 'zeroDummySequence';
end
varsCheckpointAfterDff = varsCheckpointAfterDff(cellfun(@(n) exist(n, 'var') == 1, varsCheckpointAfterDff));
try
    save(checkpointAfterDffFile, varsCheckpointAfterDff{:}, '-v7.3');
catch
    try
        save(checkpointAfterDffFile, varsCheckpointAfterDff{:});
    catch ME
        warning('VoltImg:checkpointAfterDff', 'Could not write dF/F checkpoint: %s', ME.message);
    end
end
...
for cc = 1:nConds
    for hh = 1:nHolos(cc)
        ...
        set(gcf, 'Position', [100, 100, 2000, 420]);
        clf
