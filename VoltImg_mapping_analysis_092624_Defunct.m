...
% ephysFilePath = char(uigetdir('/Volumes/Untitled/Voltage Imaging/voltMapping/Ephys')); % Select and set root folder where all experiments with cells you want to analyze are located
...
% ImgsFilePath = char(uigetdir('/Volumes/Untitled/Voltage Imaging/voltMapping/Imaging')); % Select and set root folder where all experiments with cells you want to analyze are located
...
%% Calculate noise subtraction area mask
% Step 1: Calculate mask for noise subtraction
figure(13); clf; imagesc(meanFluorMaxDvStack); axis equal; axis image; colorbar; set(gca, 'fontsize', 12);% caxis([-7 -4]);

%%%%%
% Hand select cell or area of interest
noiseRoiX = []; noiseRoiY = [];
noiseRoiHandSelect = drawfreehand;
noiseRroiHandSelectMask = createMask(noiseRoiHandSelect);
[noiseRoiX, noiseRoiY] = find(noiseRroiHandSelectMask);

% Calculate mean fluorescence of the area of interest and then form into final ROI mask

noiseRoiMeanMaxDvStack = zeros(size(maxDvStack, 1), size(maxDvStack, 2));
for rr = 1:length(noiseRoiX)
    noiseRoiMeanMaxDvStack(noiseRoiX(rr), noiseRoiY(rr)) = mean(maxDvStack(noiseRoiX(rr), noiseRoiY(rr),:), 3);
end

figure(11); clf;
imagesc(noiseRoiMeanMaxDvStack); axis equal; axis image; colorbar;

% Old version: Calculate mean fluorescence of the area of interest and then form into final ROI mask
% roiMeanMaxDvStack = mean(maxDvStack(min(roiX):max(roiX), roiY, :), 3);
% imagesc(roiMeanMaxDvStack)

stdFluor = std(nonzeros(noiseRoiMeanMaxDvStack));
meanFluor = mean(nonzeros(noiseRoiMeanMaxDvStack));

% Designate cutoff fluorescence for pixels to be selected for ROI
% noiseCutOffFluor = meanFluor; %stdFluor*1+ meanFluor; % currently cutoff is 1 standard devs from mean fluorescence
% 
% noiseRoiMeanMaxDvStack(noiseRoiMeanMaxDvStack <= noiseCutOffFluor) = 0;
% noiseRoiMeanMaxDvStack(noiseRoiMeanMaxDvStack > 0) = 1;
noiseRoiStack = noiseRoiMeanMaxDvStack;

figure(12); clf;
imagesc(noiseRoiStack); axis equal; axis image;
[noiseRoiX, noiseRoiY] = find(noiseRoiStack);

voltMapping.noiseRoiMeanMaxDvStack = noiseRoiMeanMaxDvStack;
voltMapping.noiseCutOffFluor = noiseCutOffFluor;
voltMapping.noiseRoiX = noiseRoiX;
voltMapping.noiseRoiY = noiseRoiY;
...
%% (NOISE SUBRACTED ver.) Calculate df and mean df for all conditions
% Step 1: Calculate df across all trials across all conditions
dfAllTrialsCondSorted = cell(nConds, 1);
noisedfAllTrialsCondSorted = cell(nConds,1);
dfAllTrialsNonSorted = cell(nTrials, 1);
noisedfAllTrialsNonSorted = cell(nTrials, 1);
...
%    Extract the pixel intensity values (new option 2)    
    ...
    for ff = 2:size(imageStack, 3)
        ...
        previousFrame = imageStack(:, :, ff-1);
            ...
            for rr = 1:size(noiseRoiX, 1)
                noiseRoiPixels(rr) = currentFrame(noiseRoiX(rr), noiseRoiY(rr));
                noisePreviousRoiPixels(rr) = previousFrame(noiseRoiX(rr), noiseRoiY(rr));
            end
        noiseIntensityChange = mean(abs(noiseRoiPixels(:) - noisePreviousRoiPixels(:)));
        intensityChange = mean(abs(roiPixels(:) - previousRoiPixels(:)));
        % df = [df; intensityChange];
        df = [df; intensityChange];
        noisedf = [noisedf; noiseIntensityChange];
    end

    df = (df - mean(df(ceil(imagingFreq*f0Start):floor(imagingFreq*f0End))))/mean(df(ceil(imagingFreq*f0Start):floor(imagingFreq*f0End)))*100;
    noisedf = (noisedf - mean(noisedf(ceil(imagingFreq*f0Start):floor(imagingFreq*f0End))))/mean(noisedf(ceil(imagingFreq*f0Start):floor(imagingFreq*f0End)))*100;
    if UpOrDown == '2'
        dfAllTrialsCondSorted{voltMapping.trialCond(tt, 1), 1} = [dfAllTrialsCondSorted{voltMapping.trialCond(tt, 1), 1}, -df];
        dfAllTrialsNonSorted{tt} = -df;
        noisedfAllTrialsCondSorted{voltMapping.trialCond(tt, 1), 1} = [dfAllTrialsCondSorted{voltMapping.trialCond(tt, 1), 1}, -noisedf];
        noisedfAllTrialsNonSorted{tt} = -noisedf;
    elseif UpOrDown =='1'
        dfAllTrialsCondSorted{voltMapping.trialCond(tt, 1), 1} = [dfAllTrialsCondSorted{voltMapping.trialCond(tt, 1), 1}, df];
        dfAllTrialsNonSorted{tt} = df;
        noisedfAllTrialsCondSorted{voltMapping.trialCond(tt, 1), 1} = [dfAllTrialsCondSorted{voltMapping.trialCond(tt, 1), 1}, noisedf];
...
voltMapping.dfAllTrialsCondSorted = dfAllTrialsCondSorted;
voltMapping.dfAllTrialsNonSorted = dfAllTrialsNonSorted;
voltMapping.noisedfAllTrialsCondSorted = noisedfAllTrialsCondSorted;
voltMapping.noisedfAllTrialsNonSorted = noisedfAllTrialsNonSorted;
...
holoSortedNoiseAllTrials = cell(nConds, 1);
for cc = 1:nConds
    holoSortedImagingAllTrials{cc} = cell(nHolos(cc), 1);
    holoSortedNoiseAllTrials{cc} = cell(nHolos(cc), 1);
end
...
for tt = 1:nTrials
    ...
    for hh = 1:nHolos(voltMapping.trialCond(tt, 1))
        if isempty(dfAllTrialsNonSorted{tt})
            dfThisHolo = NaN(ceil((ipi*nPulses+preStimWindow+postStimWindow)/1000*imagingFreq)+1, 1);
            noisedfThisHolo = NaN(ceil((ipi*nPulses+preStimWindow+postStimWindow)/1000*imagingFreq)+1, 1);
        else
            ...
            noisedfThisHolo = noisedfAllTrialsNonSorted{tt}(ceil((voltMapping.outParams.firstStimTimes{voltMapping.trialCond(tt, 1)}(hh)-preStimWindow/1000)*imagingFreq):(ceil(voltMapping.outParams.firstStimTimes{voltMapping.trialCond(tt, 1)}(hh)*imagingFreq)+ceil((ipi*nPulses+postStimWindow)/1000*imagingFreq)));
            noisedfThisHolo = noisedfThisHolo - mean(noisedfThisHolo((ipi+preStimWindow)/1000*imagingFreq-2:(ipi+preStimWindow)/1000*imagingFreq));
        end
...
%             holoSortedImagingAllTrials{voltMapping.trialCond(tt, 1)}{hh, 1}(:, tt) = NaN(ceil(ipi*nPulses/1000*imagingFreq)+1, 1);
            ...
            holoSortedNoiseAllTrials{voltMapping.trialCond(tt, 1)}{sortingIndex(hh), 1} = [holoSortedNoiseAllTrials{voltMapping.trialCond(tt, 1)}{sortingIndex(hh), 1}, NaN(ceil((ipi*nPulses+preStimWindow+postStimWindow)/1000*imagingFreq)+1, 1);];        
        else
%         holoSortedImagingAllTrials{voltMapping.trialCond(tt, 1)}{holoSeqIndex{voltMapping.trialCond(tt, 1)}(hh, tt), 1}(:, tt) = dfThisHolo;
            ...
            holoSortedNoiseAllTrials{voltMapping.trialCond(tt, 1)}{sortingIndex(hh), 1} = [holoSortedNoiseAllTrials{voltMapping.trialCond(tt, 1)}{sortingIndex(hh), 1}, noisedfThisHolo];
        end
    end
end
...
holoSortedImagingAllTrials_ALT = holoSortedImagingAllTrials;
holoSortedNoiseAllTrials_ALT = holoSortedNoiseAllTrials;
for cc = 1:nConds
    for hh = 1:nHolos(cc)
        for tt = 1:size(holoSortedDataAllTrials{cc}{hh}, 2)
            if isnan(holoSortedDataAllTrials{cc}{hh}(:, tt))
                holoSortedImagingAllTrials_ALT{cc}{hh}(:, tt) = NaN;
                holoSortedNoiseAllTrials_ALT{cc}{hh}(:, tt) = NaN;
            end
        end
        ...
        holoSortedNoiseAllTrials_ALT{cc}{hh} = holoSortedNoiseAllTrials_ALT{cc}{hh}(:, nonNanColumns);
    end
end  


holoSortedImagingMean = cell(nConds, 1);
holoSortedNoiseMean = cell(nConds, 1);
for cc = 1:nConds
    for hh = 1:nHolos(cc)
        holoSortedImagingMean{cc}(:, hh) = nanmean(holoSortedImagingAllTrials_ALT{cc}{hh}, 2);
        holoSortedNoiseMean{cc}(:, hh) = nanmean(holoSortedNoiseAllTrials_ALT{cc}{hh}, 2);
        % Baseline the mean holo traces
        ...
        holoSortedNoiseMean{cc}(:, hh) = holoSortedNoiseMean{cc}(:, hh) - mean(holoSortedNoiseMean{cc}((ipi+preStimWindow)/1000*imagingFreq-2:(ipi+preStimWindow)/1000*imagingFreq, hh));
