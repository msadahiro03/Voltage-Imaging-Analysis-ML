%% Voltage Imaging Mapping Analysis 021226 newDFFcalc and MCfineROI
% New version with:
% 1) Per-trial NoRMCorre motion correction of raw image stacks, saved to a
%    separate folder.
% 2) maxDvStack: per-trial mean planes (NaN for ephys-excluded and for trials not in the random 50%%
%    subsample of eligible trials); each plane uses the first max(1,floor(imagingFreq*4)) MC frames only.
%    Third dim is trial index nTrials. Grand mean uses 'omitnan'.
...
% maxDvStack sampling: 50%% of eligible (non-excluded) trials at random; first 4s of frames for each mean plane
