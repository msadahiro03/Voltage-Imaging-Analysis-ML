...
ephysFilePath = char(uigetdir('/Volumes/Untitled/Data/Voltage Imaging/voltMapping/Ephys Data')); % Select and set root folder where all experiments with cells you want to analyze are located
...
ImgsFilePath = char(uigetdir('/Volumes/Untitled/Data/Voltage Imaging/voltMapping/Imaging Data')); % Select and set root folder where all experiments with cells you want to analyze are located
...
nConds = length(unique(ExpStruct.trialCond)); % total number of powers used
...
% Step 4: Identify GEVI type (some GEVI are the positive fluorescence type, others are the dimming type)
UpOrDown = input('1 for upward GEVI, 2 for downward GEVI: ', 's'); % ASAP5, 7, and Jedi2P are 'downward'; ASAP6 and 8 are 'upward'
...
maxDvStack = [];
for tt = 1:50%length(imagesIndex)
