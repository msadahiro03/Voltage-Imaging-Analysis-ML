...
preStimWindow = 25; % time(ms) window before first stim 
...
%     end
    ...
    % Now break apart the imageStack for this trial into stim windows and rearrange according to hologram sequence
%     sortingIndex = [];
...
%         framesThisHolo = imageStack(:, :, ceil((voltMapping.outParams.firstStimTimes{voltMapping.trialCond(tt, 1)}(hh)-preStimWindow/1000)*imagingFreq):(ceil(voltMapping.outParams.firstStimTimes{voltMapping.trialCond(tt, 1)}(hh)*imagingFreq)+ceil((ipi*nPulses+postStimWindow)/1000*imagingFreq)));
        framesThisHolo = imageStack(:, :, ceil((voltMapping.outParams.firstStimTimes{voltMapping.trialCond(tt, 1)}(hh)-preStimWindow/1000)*imagingFreq):(ceil((voltMapping.outParams.firstStimTimes{voltMapping.trialCond(tt, 1)}(hh)-preStimWindow/1000)*imagingFreq)+ceil((ipi*nPulses+(preStimWindow+postStimWindow))/1000*imagingFreq)));
        ...
        for ff = 1:size(framesThisHolo, 3)
            ...
            % Calculate df
            intensityChange = currFrameRoiMean - roiBaselineMean; % essentially, intensity change = df/f
...
voltMapping.mouseID = ['voltMapping_Analysis_', expID, '_newWithExclusion'];
